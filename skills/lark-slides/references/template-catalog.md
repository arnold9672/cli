...
3. 先看模板下方的 **页型索引**，锁定你真正需要的页型：封面 / 目录 / 分节 / 内容 / 结尾
4. 仅按需读取对应模板 XML 的相关 slide 范围；只有确定要复用复杂布局时，再继续展开相邻片段
5. 从模板中提取并复用：`<theme>` 配色、页面流、shape 排列布局、装饰元素风格
6. 将用户的实际内容填充到模板的结构框架中，**不要照搬模板的占位文字**
