...
// Set once during Factory initialization (cmdutil/factory_default.go)
// and may be overridden by config bind's RunE (FR-038).
// Uses atomic.Value for goroutine safety (background registry refresh reads
// GetRuntimeDir concurrently with Factory init that writes workspace).
